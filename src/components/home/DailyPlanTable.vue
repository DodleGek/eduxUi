<template>
  <el-card shadow="hover" class="user-table">
    <div class="card-header">
      <span>我的每日计划</span>
      <el-button type="primary" size="small" @click="$emit('add-plan')"
        >添加计划</el-button
      >
    </div>
    <el-table :data="plans" style="width: 100%">
      <!-- 修改这里，使用Object.entries或明确的列定义 -->
      <el-table-column
        v-for="(val, prop) in columnsToShow"
        :key="prop"
        :prop="prop"
        :label="val"
      >
        ...
        <template #default="scope">
          <div class="checkbox-wrapper" @click="togglePlanStatus(scope.row)">
            <div
              ...
              :class="{ 'is-checked': scope.row.status === '已完成' }"
            >
